                               ...
                               const error_history_count_t * errhData )
{
    ...
    {
      i_parser.PrintHeading(i_name);
      i_parser.PrintString("ERRHIST Counts",l_stream4xml.str().c_str());
      i_parser.PrintSectionEnd(true);
    }
}
                                   ...
                                   const uint8_t i_channelFuncId )
{
    ...
    if(!l_isxmlparser)
    {
      ...
      l_sensor << " FID:["<<setfill('0')<<setw(2)<<channel<<"]";
      l_sensor << " Sample:["<<setfill('0')<<setw(5)<<ntohs(i_sensor.sample)<<"]";
      l_sensor << " Avg:["<<setfill('0')<<setw(5)<<ntohs(i_sensor.average)<<"]";
      l_sensor << " Min:["<<setfill('0')<<setw(5)<<ntohs(i_sensor.sampleMin)<<"]";
      l_sensor << " Max:["<<setfill('0')<<setw(5)<<ntohs(i_sensor.sampleMax)<<"]" << " |\n";
...
// Do sanity check on the function inputs
    if (sizeof(ChomLogData_v0_t) == i_len)
    {
        do
        {
            ...
            i_parser.PrintNumber("Number of Samples", "%d", ntohl(i_chom->sensorData[i].pwrMode.numOfSamples));
            // Node total power (DC)
                                  ...
                                  uint8_t i_mode_idx)
{
    ...
    i_parser.PrintNumber("Number of Samples", "%d", ntohl(i_sensor_data->pwrMode.numOfSamples));
    ...
    i_parser.PrintString("Processor Frequency", "");
    PrintSensorData(i_parser,"CHOMFREQP0",i_sensor_data->sensor[VER_1::CHOMFREQP0]);
    PrintSensorData(i_parser,"CHOMFREQP1",i_sensor_data->sensor[VER_1::CHOMFREQP1]);
    PrintSensorData(i_parser,"CHOMFREQP2",i_sensor_data->sensor[VER_1::CHOMFREQP2]);
    PrintSensorData(i_parser,"CHOMFREQP3",i_sensor_data->sensor[VER_1::CHOMFREQP3]);
    i_parser.PrintDivider();
    i_parser.PrintSectionEnd(true);
    ...
    i_parser.PrintString("Processor utilization", "");
    PrintSensorData(i_parser,"CHOMUTILP0", i_sensor_data->sensor[VER_1::CHOMUTILP0]);
    PrintSensorData(i_parser,"CHOMUTILP1", i_sensor_data->sensor[VER_1::CHOMUTILP1]);
    PrintSensorData(i_parser,"CHOMUTILP2", i_sensor_data->sensor[VER_1::CHOMUTILP2]);
    PrintSensorData(i_parser,"CHOMUTILP3", i_sensor_data->sensor[VER_1::CHOMUTILP3]);
    i_parser.PrintDivider();
    i_parser.PrintSectionEnd(true);
    ...
    i_parser.PrintString("Memory bandwidth", "");
    PrintSensorData(i_parser,"CHOMBWP0M0", i_sensor_data->sensor[VER_1::CHOMBWP0M0]);
    PrintSensorData(i_parser,"CHOMBWP0M1", i_sensor_data->sensor[VER_1::CHOMBWP0M1]);
    PrintSensorData(i_parser,"CHOMBWP0M2", i_sensor_data->sensor[VER_1::CHOMBWP0M2]);
    PrintSensorData(i_parser,"CHOMBWP0M3", i_sensor_data->sensor[VER_1::CHOMBWP0M3]);
    PrintSensorData(i_parser,"CHOMBWP0M4", i_sensor_data->sensor[VER_1::CHOMBWP0M4]);
    PrintSensorData(i_parser,"CHOMBWP0M5", i_sensor_data->sensor[VER_1::CHOMBWP0M5]);
    PrintSensorData(i_parser,"CHOMBWP0M6", i_sensor_data->sensor[VER_1::CHOMBWP0M6]);
    PrintSensorData(i_parser,"CHOMBWP0M7", i_sensor_data->sensor[VER_1::CHOMBWP0M7]);
    PrintSensorData(i_parser,"CHOMBWP1M0", i_sensor_data->sensor[VER_1::CHOMBWP1M0]);
    PrintSensorData(i_parser,"CHOMBWP1M1", i_sensor_data->sensor[VER_1::CHOMBWP1M1]);
    PrintSensorData(i_parser,"CHOMBWP1M2", i_sensor_data->sensor[VER_1::CHOMBWP1M2]);
    PrintSensorData(i_parser,"CHOMBWP1M3", i_sensor_data->sensor[VER_1::CHOMBWP1M3]);
    PrintSensorData(i_parser,"CHOMBWP1M4", i_sensor_data->sensor[VER_1::CHOMBWP1M4]);
    PrintSensorData(i_parser,"CHOMBWP1M5", i_sensor_data->sensor[VER_1::CHOMBWP1M5]);
    PrintSensorData(i_parser,"CHOMBWP1M6", i_sensor_data->sensor[VER_1::CHOMBWP1M6]);
    PrintSensorData(i_parser,"CHOMBWP1M7", i_sensor_data->sensor[VER_1::CHOMBWP1M7]);
    PrintSensorData(i_parser,"CHOMBWP2M0", i_sensor_data->sensor[VER_1::CHOMBWP2M0]);
    PrintSensorData(i_parser,"CHOMBWP2M1", i_sensor_data->sensor[VER_1::CHOMBWP2M1]);
    PrintSensorData(i_parser,"CHOMBWP2M2", i_sensor_data->sensor[VER_1::CHOMBWP2M2]);
    PrintSensorData(i_parser,"CHOMBWP2M3", i_sensor_data->sensor[VER_1::CHOMBWP2M3]);
    PrintSensorData(i_parser,"CHOMBWP2M4", i_sensor_data->sensor[VER_1::CHOMBWP2M4]);
    PrintSensorData(i_parser,"CHOMBWP2M5", i_sensor_data->sensor[VER_1::CHOMBWP2M5]);
    PrintSensorData(i_parser,"CHOMBWP2M6", i_sensor_data->sensor[VER_1::CHOMBWP2M6]);
    PrintSensorData(i_parser,"CHOMBWP2M7", i_sensor_data->sensor[VER_1::CHOMBWP2M7]);
    PrintSensorData(i_parser,"CHOMBWP3M0", i_sensor_data->sensor[VER_1::CHOMBWP3M0]);
    PrintSensorData(i_parser,"CHOMBWP3M1", i_sensor_data->sensor[VER_1::CHOMBWP3M1]);
    PrintSensorData(i_parser,"CHOMBWP3M2", i_sensor_data->sensor[VER_1::CHOMBWP3M2]);
    PrintSensorData(i_parser,"CHOMBWP3M3", i_sensor_data->sensor[VER_1::CHOMBWP3M3]);
    PrintSensorData(i_parser,"CHOMBWP3M4", i_sensor_data->sensor[VER_1::CHOMBWP3M4]);
    PrintSensorData(i_parser,"CHOMBWP3M5", i_sensor_data->sensor[VER_1::CHOMBWP3M5]);
    PrintSensorData(i_parser,"CHOMBWP3M6", i_sensor_data->sensor[VER_1::CHOMBWP3M6]);
    PrintSensorData(i_parser,"CHOMBWP3M7", i_sensor_data->sensor[VER_1::CHOMBWP3M7]);
    i_parser.PrintDivider();
    i_parser.PrintSectionEnd(true);
}
                                     ...
                                     uint8_t i_modes )
{
    ...
    if (sizeof(ChomLogData_v1_t) == i_len)
    {
        do
        {
            ...
            i_parser.PrintHeading("Slave Error History Counts");
                                     ...
                                     uint8_t i_modes )
{
    ...
    if(sizeof(ChomLogData_v2_t) == i_len)
    {
        do
        {
            ...
            i_parser.PrintHeading("Slave Frequency Clip History");
...
static void PrintCallHomeData(ErrlUsrParser & i_parser, void * i_buffer, uint32_t i_len)
{
    ...
    i_parser.PrintNumber("Total Time (seconds)", "%d", ntohl(l_chom->nodeData.totalTime));
    i_parser.PrintNumber("Modes in Log", "%d", l_chom->nodeData.modeInLog);
    i_parser.PrintNumber("Data Length", "%d", i_len);
