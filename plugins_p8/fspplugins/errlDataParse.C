...
/*****************************************************************************/
void parseCorrDataEntry (ErrlUsrParser  &i_parser, corrosionData_t i_corrEntry)
{
    float l_tempFloat = 0;
    float l_tempF = 0;
                    ...
                    errlsubsec_t    i_sst)
{
    ...
    else
    {
        ...
        parseCorrDataEntry(i_parser, l_corrData->header.initialReading);
        i_parser.PrintDivider();
        ...
        {
            parseCorrDataEntry(i_parser, l_corrData->twoHourReadings[l_index] );
        }
        i_parser.PrintDivider();
        ...
        {
            parseCorrDataEntry(i_parser, l_corrData->dailyAverages[l_index] );
        }
        ...
        {
            parseCorrDataEntry(i_parser, l_corrData->weeklyAverages[l_index]);
        }
...
)
{
    ...
    case ERRL_UDT_CORR: 
        l_rc = parseCorrData(i_parser, i_buffer, i_buflen, i_ver, i_sst );
